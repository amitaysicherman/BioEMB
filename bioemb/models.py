...
def get_decoder_config(tokenizer, hidden_size: int, num_hidden_layers: int,
                       num_attention_heads: int, dropout: float
...
class BioEmbModel(nn.Module):
    ...
    def __init__(
        ...
        # Generative decoder
        self.decoder = BertGenerationDecoder(get_decoder_config(
            tokenizer=tokenizer,
            hidden_size=hidden_size,
            num_hidden_layers=num_hidden_layers,
            num_attention_heads=num_attention_heads,
    ...
    def forward(
        ...
        Args:
            encoder_outputs: Hidden states from the source encoder.
        ...
        """
        # 1. Pass through bottleneck
        bottleneck = self.to_bottleneck(encoder_outputs)
        if return_bottleneck:
            return bottleneck

        projected_encoder_states = self.relu(self.from_bottleneck(bottleneck))
        # 2. Add a sequence dimension for cross-attention
        ...
        if should_constrain:
            decoder_outputs = self._apply_trie_constraints(decoder_outputs, input_ids, labels)
        ...
        return {
            "loss": decoder_outputs.loss if labels is not None else None,
